...
import {
  ICampaignTypeControllerReadCampaignTypeOutputError as IControllerError,
  ICampaignTypeControllerReadCampaignTypeOutputSuccess as IControllerSuccess,
...
/**
 * @name					campaignTypeRouterReadCampaignType
 *
 * @description		The 'readCampaignType' router for 'campaignType', whose expected
 *                inputs have been defined within the following schema:
 *                'campaignTypeValidationSchemaReadCampaignType'.
 *
...
export const campaignTypeRouterReadCampaignType = Router(options).get(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
       ...
       * defined within 'campaignTypeValidationSchemaReadCampaignType'.
      ...
      const stat = await campaignTypeController.readCampaignType(validatedParams);
       ...
       * 'campaignTypeModel' from 'stat.payload' and return
       * it with an appropriate status code.
       */

      const { campaignTypeModel, responseStatusCode } = (stat as IControllerSuccess)
        .payload;
      res.status(responseStatusCode).send({ campaignTypeModel });
    } catch (error) {
      /*

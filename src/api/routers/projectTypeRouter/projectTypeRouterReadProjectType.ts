...
import {
  IProjectTypeControllerReadProjectTypeOutputError as IControllerError,
  IProjectTypeControllerReadProjectTypeOutputSuccess as IControllerSuccess,
...
/**
 * @name					projectTypeRouterReadProjectType
 *
 * @description		The 'readProjectType' router for 'projectType', whose expected
 *                inputs have been defined within the following schema:
 *                'projectTypeValidationSchemaReadProjectType'.
 *
...
export const projectTypeRouterReadProjectType = Router(options).get(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
       ...
       * defined within 'projectTypeValidationSchemaReadProjectType'.
      ...
      const stat = await projectTypeController.readProjectType(validatedParams);
       ...
       * 'projectTypeModel' from 'stat.payload' and return
       * it with an appropriate status code.
       */

      const { projectTypeModel, responseStatusCode } = (stat as IControllerSuccess)
        .payload;
      res.status(responseStatusCode).send({ projectTypeModel });
    } catch (error) {
      /*

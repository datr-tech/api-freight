...
import {
  IProjectControllerReadProjectOutputError as IControllerError,
  IProjectControllerReadProjectOutputSuccess as IControllerSuccess,
...
/**
 * @name					projectRouterReadProject
 *
 * @description		The 'readProject' router for 'project', whose expected
 *                inputs have been defined within the following schema:
 *                'projectValidationSchemaReadProject'.
 *
...
export const projectRouterReadProject = Router(options).get(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
       ...
       * defined within 'projectValidationSchemaReadProject'.
      ...
      const stat = await projectController.readProject(validatedParams);
       ...
       * 'projectModel' from 'stat.payload' and return
       * it with an appropriate status code.
       */

      const { projectModel, responseStatusCode } = (stat as IControllerSuccess).payload;
      res.status(responseStatusCode).send({ projectModel });
    } catch (error) {
      /*

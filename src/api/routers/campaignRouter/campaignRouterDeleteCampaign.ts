...
import {
  ICampaignControllerDeleteCampaignOutputError as IControllerError,
  ICampaignControllerDeleteCampaignOutputSuccess as IControllerSuccess,
...
/**
 * @name					campaignRouterDeleteCampaign
 *
 * @description		The 'deleteCampaign' router for 'campaign', whose expected
 *                inputs have been defined within the following schema:
 *                'campaignValidationSchemaDeleteCampaign'.
 *
 ...
 *                controller, 'campaignController', which, when successful, will return
 ...
 *                | Server error                | 500               |
 */
export const campaignRouterDeleteCampaign = Router(options).delete(
  '/',
  ...
  async (req: Request, res: Response) => {
    ...
    try {
       ...
       * defined within 'campaignValidationSchemaDeleteCampaign'.
      ...
      const stat = await campaignController.deleteCampaign(validatedParams);
       ...
       * 'campaignId' from 'stat.payload' and return
      ...
      res
        ...
        .send({ campaignId: controllerResponsePayload['campaignId'] });
    } catch (error) {
      /*
